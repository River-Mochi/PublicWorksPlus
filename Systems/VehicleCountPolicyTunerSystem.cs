...
// - If Public Works Plus is loaded and its transit-line tuner is enabled, ATC skips this system
//   to avoid both mods fighting over the same VehicleCountPolicy.
...
namespace AdjustTransit
{
    ...
    using System;                          // AppDomain, Exception
    using System.Reflection;               // BindingFlags, FieldInfo, PropertyInfo
    ...
    public sealed partial class VehicleCountPolicyTunerSystem : GameSystemBase
    {
        ...
        private static bool s_LoggedSkipDueToPwP;
        private static bool s_LoggedPwPReflectionWarning;
        ...
        protected override void OnUpdate()
        {
            ...
            if (IsPwPTransitLineTunerActive(verbose))
            {
                if (!s_LoggedSkipDueToPwP || verbose)
                {
        ...
        private static bool IsPwPTransitLineTunerActive(bool verbose)
        {
            try
            {
                foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
                {
                    Type modType = assembly.GetType("PublicWorksPlus.Mod", throwOnError: false);
                    if (modType == null)
                    {
                        continue;
                    }

                    FieldInfo settingsField = modType.GetField("Settings", BindingFlags.Public | BindingFlags.Static);
                    if (settingsField == null)
                    {
                        WarnPwPReflectionFallback("Settings field not found on PublicWorksPlus.Mod.");
                        return true;
                    }

                    object settingsObject = settingsField.GetValue(null);
                    if (settingsObject == null)
                    {
                        WarnPwPReflectionFallback("Public Works Plus Settings is null; assuming tuner may be enabled.");
                        return true;
                    }

                    PropertyInfo tunerProperty = settingsObject.GetType().GetProperty(
                        "EnableLineVehicleCountTuner",
                        BindingFlags.Public | BindingFlags.Instance);

                    if (tunerProperty == null || tunerProperty.PropertyType != typeof(bool))
                    {
                        WarnPwPReflectionFallback("EnableLineVehicleCountTuner property not found on Public Works Plus settings.");
                        return true;
                    }

                    bool enabled = (bool)tunerProperty.GetValue(settingsObject);

                    if (verbose)
                    {
                        Mod.s_Log.Info(
                            $"{Mod.ModTag} VehicleCountPolicyTuner: detected Public Works Plus loaded; transit-line tuner enabled={enabled}.");
                    }
            ...
            {
                WarnPwPReflectionFallback(
                    $"Exception while checking Public Works Plus state: {ex.GetType().Name}: {ex.Message}");
                return true;
            }
        }

        private static void WarnPwPReflectionFallback(string message)
        {
            if (!s_LoggedPwPReflectionWarning)
            {
                s_LoggedPwPReflectionWarning = true;
                Mod.s_Log.Warn(
                    $"{Mod.ModTag} VehicleCountPolicyTuner: {message} " +
                    "ATC will skip its transit-line tuner as a safety fallback.");
            }
        }
        ...
        private static Bounds1 BuildInverseRelativeInputRange(float fewerVehiclesApplied, float moreVehiclesApplied)
        {
            ...
            {
                (inputMax, inputMin) = (inputMin, inputMax);
            }
